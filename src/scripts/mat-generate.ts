...
const days = Object.keys(baseTimetable);
...
async function main() {
  try {
    ...
    if (labTimetables.length === 0) {
      console.log("❌ No lab timetables found");
      return;
    }
    ...
    if (!matCourseData) {
      console.log("❌ MAT1002 course not found");
      return;
    }
    ...
    console.log("🎉 Done generating lab+MAT timetables!");
  } catch (error) {
