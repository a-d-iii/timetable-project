...
// Base timetable layout (used solely for mapping positions)
...
/**
 * Normalize a slot code – for example, treat "TDDI" as "tdd1".
 */
...
/**
 * Build a mapping from each normalized slot code to its positions (day and index)
...
/**
 * Overlays new slot codes (with a given label) onto an existing grid.
 * Only cells that are exactly empty ("") are filled.
 */
...
/**
 * Generator for CSE possibilities from the CSE2005 course.
 */
...
/**
 * Process a single timetable record: fetch its details, check CSE possibilities,
 * insert new combined records immediately, and log the new record IDs.
 */
async function processTimetableRecord(recordId: number, csePossibilities: SlotCombo[], cseCourse: Course): Promise<number> {
  ...
  if (record.grid) {
    if (typeof record.grid === "string") {
      storedGrid = JSON.parse(record.grid);
    } else {
      storedGrid = record.grid as Record<string, string[]>;
    }
  } else {
...
/**
 * POST handler that processes timetable records in large batches concurrently.
 * Batch size is set to 50,000.
 */
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({
      message: "Lab+MAT+ECE+CSE timetable combinations generated successfully",
      count: totalNew,
    });
  } catch (error) {
    console.error("Error generating Lab+MAT+ECE+CSE timetables:", error);
    return NextResponse.json({ message: "Error generating Lab+MAT+ECE+CSE timetables" }, { status: 500 });
  }
}
