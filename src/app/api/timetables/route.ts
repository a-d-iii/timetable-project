...
// Fixed base timetable layout: 13 cells per day (same for all, no extra lab logic now)
...
/**
 * Build a global mapping from each slot code (lowercase) to all its positions (day, index)
 * from the base timetable.
 */
 ...
 * where each combination includes one slotCombo from every course.
 */
function* generateTimetablesGenerator(
  ...
  for (const slotCombo of course.slotCombos) {
    const codes = slotCombo.slotCode.split("+").map(code => code.trim().toLowerCase());
    // Reject if any code is already used.
...
/**
 * POST handler to generate timetable combinations for lab courses only.
 * - Clears existing timetable records.
 * - Fetches all courses.
 * - Filters to only lab courses (course code ending with "l").
 * - Generates all valid combinations using the generator.
 * - Inserts each combination (with computed grid) into the database.
 * - Returns the total number of combinations generated.
 */
export async function POST(req: Request) {
  try {
    ...
    // Use the generator to produce all valid combinations.
    const combinations: SlotCombo[][] = [];
    for (const combo of generateTimetablesGenerator(courses)) {
      combinations.push(combo);
    }

    // For testing, return the count (or optionally insert into DB).
    // If you want to test up to, say, 50k combinations, you could check combinations.length.
    // Here we just insert them.
    for (const timetableComb of combinations) {
